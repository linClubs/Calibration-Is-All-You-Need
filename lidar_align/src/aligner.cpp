...
std::string Aligner::generateCalibrationString(const Transform& T,
  ...
  // ----------转成角度制查看-----------------------
  Eigen::Vector3d A = Eigen::Vector3d(T_log[3],T_log[4], T_log[5]);
  Eigen::Vector3d A_norm = A.normalized();
  double len = A.norm();

  Eigen::AngleAxisd angle = Eigen::AngleAxisd(len, A_norm);
  Eigen::Vector3d euler1 = angle.matrix().eulerAngles(2,1,0);
  
  euler1  = euler1 * 180 / M_PI;
  
  // std::cout << "--AngleAxisd2Euler(RPY): " <<  euler1;

  ss << "\n--------------\n\n--AngleAxisd2Euler(RPY): " << std::endl
  << euler1 << std::endl << std::endl;
  ss << "translation: " << T_log[0] << ", ";
  ss << T_log[1] << ", ";
  ss << T_log[2] << "\n";


  Eigen::Quaterniond q = Eigen::Quaterniond(T.rotation().w(), T.rotation().x(), T.rotation().y(), T.rotation().z());
  Eigen::Vector3d euler2 = q.toRotationMatrix().eulerAngles(2,1,0);
  euler2  = euler2 * 180 / M_PI;

  // std::cout << "--Rotation2Euler(RPY-Z-Y-X): " <<  euler2 << std::endl;

  ss << "\n--Rotation2Euler(RPY-Z-Y-X): " << std::endl
  << euler2 << std::endl << std::endl;
  // ---------------------------------------
  ss << "translation: " << T.translation().x() << ", ";
  ss << T.translation().y() << ", ";
  ss << T.translation().z() << "\n--------------\n\n" ;
